		...
		if (new Autenticar(new Diretor(login, senha)).autenticar()) {// se TRUE acessa, se FALSE nao acessa;
			...
			while (addAluno == 0) {
				...
				while (addDisciplinas == 0) {// condiçao para adicionar quantas disciplinas o usuario desejar. Opção de
					...
					String numDisciplinas = JOptionPane.showInputDialog(null, "Quantas disciplinas deseja adicionar?");

					for (int posi = 1; posi <= (Integer.valueOf(numDisciplinas)); posi++) {
						String nomeDisciplina = JOptionPane.showInputDialog("Qual o nome da disciplina " + posi + "?");
